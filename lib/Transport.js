...
class Transport {
  ...
  sniff (opts, callback = noop) {
    ...
    this.request(request, { id: opts.requestId }, (err, result) => {
      ...
      var url = ''
      if (Object.prototype.hasOwnProperty.call(result.meta.connection, 'url') &&
          typeof result.meta.connection.url === 'string') {
        url = result.meta.connection.url
      } else if ((Object.prototype.hasOwnProperty.call(result.meta.connection, 'url') &&
          typeof result.meta.connection.url === 'object')) {
        url = result.meta.connection.url.href
      }
      const hosts = this.connectionPool.nodesToHost(result.body.nodes, protocol, url)
