  ...
  request (params, options, callback) {
    ...
    if (this[kProductCheck] === 0 || this[kProductCheck] === 1) {
      ...
      if (params.method === 'GET' && params.path === '/') {
        ...
        this[kEventEmitter].once('product-check', status => {
          if (status === false) {
            const err = new ElasticsearchClientError('The client noticed that the server is not Elasticsearch and we do not support this unknown product.')
            this.emit('request', err, result)
            process.nextTick(callback, err, result)
          } else {
            prepareRequest()
    ...
    this.request({
      ...
      if (err) {
        if (err.statusCode === 401 || err.statusCode === 403) {
          ...
          process.emitWarning('The client is unable to verify that the server is Elasticsearch due security privileges on the server side.')
          this[kEventEmitter].emit('product-check', true)
        } else {
          this[kEventEmitter].emit('product-check', false)
        }
      } else {
        if (result.body.version == null || typeof result.body.version.number !== 'string') {
          return this[kEventEmitter].emit('product-check', false)
        }
