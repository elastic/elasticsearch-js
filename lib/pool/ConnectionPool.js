...
class ConnectionPool extends BaseConnectionPool {
  ...
  markDead (connection) {
    ...
    // If the dead connection has already been removed in sniff, we MUST not mark it
    const connectionById = this.connections.find(c => c.id === id)
    const connectionByUrl = this.connections.find(c => c.id === connection.url.href)
    if (!connectionById && !connectionByUrl) return this
