...
class BaseConnectionPool {
   ...
   * @returns {array} hosts
   */
  nodesToHost (nodes, protocol, url) {
    ...
    for (var i = 0, len = ids.length; i < len; i++) {
      ...
      // In some environments (e.g. AWS) the http property is not provided
      // in the response from the _node/_all/http. In this case
      // the connection url is used.
      var address = url
      if (Object.prototype.hasOwnProperty.call(node, 'http') &&
          Object.prototype.hasOwnProperty.call(node.http, 'publish_address')) {
        address = node.http.publish_address
        const parts = address.split('/')
        console.log(`parts size = ${parts.toString()}`)
