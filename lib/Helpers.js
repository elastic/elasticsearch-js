...
class Helpers {
   ...
   */
  bulk (opts) {
    // TODO: add refreshOnCompletion option
    // TODO: any option passed to the helper that is not needed
    //       by the helper, should be passed to the bulk api
    //       eg: pipeline. This could also be handled nicely
    //       in ts by extending the bulk type definition.
    ...
    const onDropFn = opts.onDrop || noop
    const onDocumentFn = opts.onDocument
    ...
    async function iterate () {
      ...
      for await (const chunk of datasource) {
        ...
        const action = onDocumentFn(chunk)
