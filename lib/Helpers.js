...
class Helpers {
    ...
    while (response.body.hits && response.body.hits.hits.length > 0) {
      ...
      for (let i = 0; i <= maxRetries; i++) {
        response = await this[kClient].scroll({
          ...this.filterQueryStringForScrolling(querystring),
          body: { scroll_id }
        }, options)
        if (response.statusCode !== 429) break
        await sleep(wait)
  ...
  filterQueryStringForScrolling (originalQueryString) {
    const { scroll, rest_total_hits_as_int, restTotalHitsAsInt } = originalQueryString;
    return ( scroll, rest_total_hits_as_int, restTotalHitsAsInt );
  }
