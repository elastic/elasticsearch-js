...
class Helpers {
  ...
  msearch (options = {}) {
    ...
    const { serialize } = client.serializer
    const {
      operations = 20,
      concurrency = 5,
    ...
    return {
      ...
      search (header, body, callback) {
        if (stopReading === true) {
          const error = new ConfigurationError('The msearch processor has been stopped')
        ...
        datasource.push([header, body, callback])
    ...
    async function iterate () {
      ...
      for await (const chunk of datasource) {
        header = typeof chunk[0] === 'string' ? chunk[0] : serialize(chunk[0])
        body = typeof chunk[1] === 'string' ? chunk[1] : serialize(chunk[1])
        chunkOperations += 1
        bulkBody.push(header)
        bulkBody.push(body)
        callbacks.push(chunk[2])
        if (chunkOperations >= operations) {
          const send = await semaphore()
          send(bulkBody.slice(), callbacks.slice())
          bulkBody.length = 0
          callbacks.length = 0
          chunkOperations = 0
        }
    ...
    function buildSemaphore () {
      ...
      function send (bulkBody, callbacks) {
        /* istanbul ignore if */
        if (running >= concurrency) {
          throw new Error('Max concurrency reached')
        }
        running += 1
        msearchOperation(bulkBody, callbacks, () => {
          running -= 1
          if (resolveSemaphore) {
    ...
    function msearchOperation (bulkBody, callbacks, done) {
      ...
      tryMsearch(bulkBody, callbacks, retrySearch)
      function retrySearch (bulkBody, callbacks) {
        if (bulkBody.length > 0 && retryCount > 0) {
          retryCount -= 1
          setTimeout(tryMsearch, wait, bulkBody, callbacks, retrySearch)
          return
        }
      ...
      function tryMsearch (bulkBody, callbacks, done) {
        client.msearch(Object.assign({}, msearchOptions, { body: bulkBody }), (err, results) => {
          const retryBody = []
          ...
          for (let i = 0, len = responses.length; i < len; i++) {
            ...
            if (response.status === 429 && retryCount > 0) {
              retryBody.push(bulkBody[i * 2])
              retryBody.push(bulkBody[(i * 2) + 1])
