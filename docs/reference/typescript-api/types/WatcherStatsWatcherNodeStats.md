...
|----------|------|-------------|
| `current_watches?` | `WatcherStatsWatchRecordStats[]` | The current executing watches metric gives insight into the watches that are currently being executed by Watcher.
...
| `queued_watches?` | `WatcherStatsWatchRecordQueuedStats[]` | Watcher moderates the execution of watches such that their execution won't put too much pressure on the node and its resources.
...
| `watch_count` | `long` | The number of watches currently registered. |
