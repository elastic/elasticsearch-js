...
|----------|------|-------------|
| `doc_count` | [`integer`](integer.md) | The total number of documents that initially created the expectations. It’s required to be greater
...
| `expectations` | [`double`](double.md)[] | An array of numbers with which to correlate the configured `bucket_path` values.
The length of this value must always equal the number of buckets returned by the `bucket_path`. |
| `fractions?` | [`double`](double.md)[] | An array of fractions to use when averaging and calculating variance. This should be used if
