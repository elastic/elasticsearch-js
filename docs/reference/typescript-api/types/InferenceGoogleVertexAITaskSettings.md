...
| `top_n?` | `integer` | For a `rerank` task, the number of the top N documents that should be returned. |
...
| `max_tokens?` | `integer` | For `completion` and `chat_completion` tasks, specifies the `max_tokens` value for requests sent to the Google Model Garden `anthropic` provider.
