...
| `max_children?` | [`integer`](integer.md) | Maximum number of child documents that match the query allowed for a returned parent document.
If the parent document exceeds this limit, it is excluded from the search results. |
| `min_children?` | [`integer`](integer.md) | Minimum number of child documents that match the query required to match the query for a returned parent document.
If the parent document does not meet this limit, it is excluded from the search results. |
| `query` | [`QueryDslQueryContainer`](QueryDslQueryContainer.md) | Query you wish to run on child documents of the `type` field.
