...
|----------|------|-------------|
| `type` | `'min_hash'` | - |
| `bucket_count?` | `integer` | Number of buckets to which hashes are assigned. Defaults to `512`. |
| `hash_count?` | `integer` | Number of ways to hash each token in the stream. Defaults to `1`. |
| `hash_set_size?` | `integer` | Number of hashes to keep from each bucket. Defaults to `1`.
