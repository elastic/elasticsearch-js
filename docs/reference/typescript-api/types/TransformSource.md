...
|----------|------|-------------|
| `index` | [`Indices`](Indices.md) | The source indices for the transform. It can be a single index, an index pattern (for example, `"my-index-*""`), an
...
| `runtime_mappings?` | [`MappingRuntimeFields`](MappingRuntimeFields.md) | Definitions of search-time runtime fields that can be used by the transform. For search runtime fields all data
