...
| `download_database_on_pipeline_creation?` | `boolean` | If `true` (and if `ingest.geoip.downloader.eager.download` is `false`), the missing database is downloaded when the pipeline is created.
