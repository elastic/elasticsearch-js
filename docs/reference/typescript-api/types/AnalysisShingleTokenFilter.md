...
| `max_shingle_size?` | `SpecUtilsStringified<integer>` | Maximum number of tokens to concatenate when creating shingles. Defaults to `2`. |
| `min_shingle_size?` | `SpecUtilsStringified<integer>` | Minimum number of tokens to concatenate when creating shingles. Defaults to `2`. |
