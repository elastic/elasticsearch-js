...
|----------|------|-------------|
| `align_checkpoints?` | `boolean` | Specifies whether the transform checkpoint ranges should be optimized for performance. Such optimization can align
...
overall performance. |
| `dates_as_epoch_millis?` | `boolean` | Defines if dates in the ouput should be written as ISO formatted string or as millis since epoch. epoch_millis was
...
| `docs_per_second?` | [`float`](float.md) | Specifies a limit on the number of input documents per second. This setting throttles the transform by adding a
wait time between search requests. The default value is null, which disables throttling. |
| `max_page_search_size?` | [`integer`](integer.md) | Defines the initial page size to use for the composite aggregation for each checkpoint. If circuit breaker
exceptions occur, the page size is dynamically adjusted to a lower value. The minimum value is `10` and the
maximum is `65,536`. |
| `use_point_in_time?` | `boolean` | Specifies whether the transform checkpoint will use the Point In Time API while searching over the source index.
...
| `unattended?` | `boolean` | If `true`, the transform runs in unattended mode. In unattended mode, the transform retries indefinitely in case
