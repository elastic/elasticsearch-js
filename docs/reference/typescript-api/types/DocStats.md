...
|----------|------|-------------|
| `count` | [`long`](long.md) | Total number of non-deleted documents across all primary shards assigned to selected nodes.
This number is based on documents in Lucene segments and may include documents from nested fields. |
| `deleted?` | [`long`](long.md) | Total number of deleted documents across all primary shards assigned to selected nodes.
...
| `total_size_in_bytes` | [`long`](long.md) | Returns the total size in bytes of all documents in this stats.
