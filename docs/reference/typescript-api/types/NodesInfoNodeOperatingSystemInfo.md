...
| `available_processors` | `integer` | Number of processors available to the Java virtual machine |
| `allocated_processors?` | `integer` | The number of processors actually used to calculate thread pool size. This number can be set with the node.processors setting of a node and defaults to the number of processors reported by the OS. |
| `name` | [`Name`](Name.md) | Name of the operating system (ex: Linux, Windows, Mac OS X) |
| `pretty_name` | [`Name`](Name.md) | - |
| `refresh_interval_in_millis` | `DurationValue<UnitMillis>` | Refresh interval for the OS statistics |
