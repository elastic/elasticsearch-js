...
|----------|------|-------------|
| `expiration_time_in_millis` | [`EpochTime`](EpochTime.md)<UnitMillis> | The timestamp, in milliseconds since the Unix epoch, when Elasticsearch will delete the search and its results, even if the search is still running. |
...
| `start_time_in_millis` | [`EpochTime`](EpochTime.md)<UnitMillis> | The timestamp, in milliseconds since the Unix epoch, when the search started.
The API returns this property only for running searches. |
| `completion_status?` | [`uint`](uint.md) | The HTTP status code for the search.
