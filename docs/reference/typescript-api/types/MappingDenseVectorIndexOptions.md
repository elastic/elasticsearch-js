...
|----------|------|-------------|
| `confidence_interval?` | [`float`](float.md) | The confidence interval to use when quantizing the vectors. Can be any value between and including `0.90` and
...
| `ef_construction?` | [`integer`](integer.md) | The number of candidates to track while assembling the list of nearest neighbors for each new node.

Only applicable to `hnsw`, `int8_hnsw`, `bbq_hnsw`, and `int4_hnsw` index types. |
| `m?` | [`integer`](integer.md) | The number of neighbors each node will be connected to in the HNSW graph.
