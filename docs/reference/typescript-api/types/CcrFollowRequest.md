...
| `max_outstanding_read_requests?` | `long` | The maximum number of outstanding reads requests from the remote cluster. |
| `max_outstanding_write_requests?` | `integer` | The maximum number of outstanding write requests on the follower. |
...
retrying. |
| `max_write_buffer_count?` | `integer` | The maximum number of operations that can be queued for writing. When this limit is reached, reads from the remote cluster will be
