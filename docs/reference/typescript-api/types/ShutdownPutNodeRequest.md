...
|----------|------|-------------|
| `node_id` | [`NodeId`](NodeId.md) | The node identifier.
...
If no response is received before the timeout expires, the request fails and returns an error. |
| `type` | [`ShutdownType`](ShutdownType.md) | Valid values are restart, remove, or replace.
...
| `target_node_name?` | `string` | Only valid if type is replace.
