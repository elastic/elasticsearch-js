...
| `owner?` | `boolean` | A boolean flag that can be used to query API keys owned by the currently authenticated user.
...
This parameter cannot be used with either `id` or `name` or when `owner` flag is set to `true`. |
| `with_limited_by?` | `boolean` | Return the snapshot of the owner user's role descriptors
