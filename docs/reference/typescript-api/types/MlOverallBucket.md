...
|----------|------|-------------|
| `bucket_span` | `DurationValue<UnitSeconds>` | The length of the bucket in seconds. Matches the job with the longest bucket_span value. |
...
| `jobs` | `MlOverallBucketJob[]` | An array of objects that contain the max_anomaly_score per job_id. |
| `overall_score` | `double` | The top_n average of the maximum bucket anomaly_score per job. |
