...
| `new_index?` | [`IndexName`](IndexName.md) | Name of the index to create.
Supports date math.
...
| `lazy?` | `boolean` | If set to true, the rollover action will only mark a data stream to signal that it needs to be rolled over at the next write.
Only allowed on data streams. |
| `aliases?` | `Record<IndexName, IndicesAlias>` | Aliases for the target index.
Data streams do not support this parameter. |
| `conditions?` | [`IndicesRolloverRolloverConditions`](IndicesRolloverRolloverConditions.md) | Conditions for the rollover.
...
| `settings?` | `Record<string, any>` | Configuration options for the index.
Data streams do not support this parameter. |
