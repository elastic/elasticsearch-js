...
|----------|------|-------------|
| `_meta?` | [`Metadata`](Metadata.md) | Custom metadata for the stream, copied from the `_meta` object of the stream’s matching index template.
...
| `settings` | [`IndicesIndexSettings`](IndicesIndexSettings.md) | The settings specific to this data stream that will take precedence over the settings in the matching index
template. |
| `mappings?` | [`MappingTypeMapping`](MappingTypeMapping.md) | The mappings specific to this data stream that will take precedence over the mappings in the matching index
template. |
| `status` | [`HealthStatus`](HealthStatus.md) | Health status of the data stream.
...
| `template` | [`Name`](Name.md) | Name of the index template used to create the data stream’s backing indices.
