...
| `number_of_allocations` | `integer` | The total number of allocations this model is assigned across ML nodes. |
...
| `queue_capacity` | `integer` | Number of inference requests are allowed in the queue at a time. |
| `threads_per_allocation` | `integer` | Number of threads per allocation. |
