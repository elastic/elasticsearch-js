# InferenceAmazonBedrockServiceSettings
...
| `model` | `string` | The base model ID or an ARN to a custom model based on a foundational model.
...
| `region` | `string` | The region that your model or ARN is deployed in.
The list of available regions per model can be found in the Amazon Bedrock documentation. |
| `rate_limit?` | [`InferenceRateLimitSetting`](InferenceRateLimitSetting.md) | This setting helps to minimize the number of rate limit errors returned from Watsonx.
