...
| `size?` | `integer` | The number of top documents to return after diversification. |
| `rank_window_size?` | `integer` | The number of top documents from the nested retriever to consider for diversification. |
...
| `lambda?` | `float` | Controls the trade-off between relevance and diversity for MMR. A value of 0.0 focuses solely on diversity, while a value of 1.0 focuses solely on relevance. Required for MMR |
