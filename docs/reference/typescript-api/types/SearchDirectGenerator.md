...
Can only be `1` or `2`. |
| `max_inspections?` | `float` | A factor that is used to multiply with the shard_size in order to inspect more candidate spelling corrections on the shard level.
