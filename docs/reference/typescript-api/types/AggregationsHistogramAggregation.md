...
|----------|------|-------------|
| `extended_bounds?` | `AggregationsExtendedBounds<double>` | Enables extending the bounds of the histogram beyond the data itself. |
| `hard_bounds?` | `AggregationsExtendedBounds<double>` | Limits the range of buckets in the histogram.
...
| `interval?` | `double` | The interval for the buckets.
...
| `missing?` | `double` | The value to apply to documents that do not have a value.
By default, documents without a value are ignored. |
| `offset?` | `double` | By default, the bucket keys start with 0 and then continue in even spaced steps of `interval`.
