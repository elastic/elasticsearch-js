...
| `dictionary?` | `string` | One or more `.dic` files (e.g, `en_US.dic`, my_custom.dic) to use for the Hunspell dictionary.
...
| `lang` | `string` | Locale directory used to specify the `.aff` and `.dic` files for a Hunspell dictionary. |
| `language` | `string` | Locale directory used to specify the `.aff` and `.dic` files for a Hunspell dictionary. |
