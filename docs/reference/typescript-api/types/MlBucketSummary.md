...
|----------|------|-------------|
| `anomaly_score` | [`double`](double.md) | The maximum anomaly score, between 0-100, for any of the bucket influencers. This is an overall, rate-limited
...
| `initial_anomaly_score` | [`double`](double.md) | The maximum anomaly score for any of the bucket influencers. This is the initial value that was calculated at the
...
| `processing_time_ms` | [`DurationValue`](DurationValue.md)<UnitMillis> | The amount of time, in milliseconds, that it took to analyze the bucket contents and calculate results. |
| `result_type` | `string` | Internal. This value is always set to bucket. |
| `timestamp` | [`EpochTime`](EpochTime.md)<UnitMillis> | The start time of the bucket. This timestamp uniquely identifies the bucket. Events that occur exactly at the
