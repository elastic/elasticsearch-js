# MappingChunkRescorerChunkingSettings
...
| `max_chunk_size` | `integer` | The maximum size of a chunk in words.
...
| `overlap?` | `integer` | The number of overlapping words for chunks.
It is applicable only to a `word` chunking strategy.
This value cannot be higher than half the `max_chunk_size` value. |
| `sentence_overlap?` | `integer` | The number of overlapping sentences for chunks.
