...
|----------|------|-------------|
| `sync_interval?` | [`Duration`](Duration.md) | How often the translog is fsynced to disk and committed, regardless of write operations.
...
| `flush_threshold_size?` | [`ByteSize`](ByteSize.md) | The translog stores all operations that are not yet safely persisted in Lucene (i.e., are not
