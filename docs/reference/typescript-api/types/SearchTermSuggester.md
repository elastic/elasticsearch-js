...
Can only be `1` or `2`. |
| `max_inspections?` | [`integer`](integer.md) | A factor that is used to multiply with the shard_size in order to inspect more candidate spelling corrections on the shard level.
...
| `prefix_length?` | [`integer`](integer.md) | The number of minimal prefix characters that must match in order be a candidate for suggestions.
