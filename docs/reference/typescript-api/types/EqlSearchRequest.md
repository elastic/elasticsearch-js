...
| `fetch_size?` | `uint` | Maximum number of events to search at a time for sequence queries. |
...
| `size?` | `uint` | For basic queries, the maximum number of matching events to return. Defaults to 10 |
...
| `max_samples_per_key?` | `integer` | By default, the response of a sample query contains up to `10` samples, with one sample per unique set of join keys. Use the `size`
