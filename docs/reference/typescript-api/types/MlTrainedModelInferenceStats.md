...
|----------|------|-------------|
| `cache_miss_count` | `integer` | The number of times the model was loaded for inference and was not retrieved from the cache.
...
| `inference_count` | `integer` | The total number of times the model has been called for inference.
...
| `timestamp` | `EpochTime<UnitMillis>` | The time when the statistics were last updated. |
