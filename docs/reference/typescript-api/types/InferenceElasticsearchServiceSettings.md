...
| `num_allocations?` | [`integer`](integer.md) | The total number of allocations that are assigned to the model across machine learning nodes.
...
| `max_chunks_per_doc?` | [`integer`](integer.md) | Only for the `rerank` task type.
