...
| `start_time_in_millis?` | [`EpochTime`](EpochTime.md)<UnitMillis> | For a running search shows a timestamp when the eql search started, in milliseconds since the Unix epoch. |
| `expiration_time_in_millis?` | [`EpochTime`](EpochTime.md)<UnitMillis> | Shows a timestamp when the eql search will be expired, in milliseconds since the Unix epoch. When this time is reached, the search and its results are deleted, even if the search is still ongoing. |
