# CatCountCountRecord

## Interface

### Properties

| Property | Type | Description |
|----------|------|-------------|
| `epoch?` | `SpecUtilsStringified<EpochTime<UnitSeconds>>` | seconds since 1970-01-01 00:00:00 |
| `t?` | `SpecUtilsStringified<EpochTime<UnitSeconds>>` | seconds since 1970-01-01 00:00:00 |
| `time?` | `SpecUtilsStringified<EpochTime<UnitSeconds>>` | seconds since 1970-01-01 00:00:00 |
| `timestamp?` | [`TimeOfDay`](TimeOfDay.md) | time in HH:MM:SS |
| `ts?` | [`TimeOfDay`](TimeOfDay.md) | time in HH:MM:SS |
| `hms?` | [`TimeOfDay`](TimeOfDay.md) | time in HH:MM:SS |
| `hhmmss?` | [`TimeOfDay`](TimeOfDay.md) | time in HH:MM:SS |
| `count?` | `string` | the document count |
| `dc?` | `string` | the document count |
| `'docs.count'?` | `string` | the document count |
| `docsCount?` | `string` | the document count |

## See Also

- [All Types](./)
- [API Methods](../index.md)
