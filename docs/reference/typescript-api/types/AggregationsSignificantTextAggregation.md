...
| `min_doc_count?` | `long` | Only return values that are found in more than `min_doc_count` hits. |
...
| `shard_min_doc_count?` | `long` | Regulates the certainty a shard has if the values should actually be added to the candidate list or not with respect to the min_doc_count.
