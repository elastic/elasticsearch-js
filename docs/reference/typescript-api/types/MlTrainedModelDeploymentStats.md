...
| `nodes` | [`MlTrainedModelDeploymentNodesStats`](MlTrainedModelDeploymentNodesStats.md)[] | The deployment stats for each node that currently has the model allocated.
...
| `rejected_execution_count?` | [`integer`](integer.md) | The sum of `rejected_execution_count` for all nodes in the deployment.
...
| `start_time` | [`EpochTime`](EpochTime.md)<UnitMillis> | The epoch timestamp when the deployment started. |
