...
| `failures?` | [`BulkIndexByScrollFailure`](BulkIndexByScrollFailure.md)[] | Array of failures if there were any unrecoverable errors during the process.
...
| `slices?` | [`ReindexStatus`](ReindexStatus.md)[] | Status of each slice if the update by query was sliced |
...
| `throttled_until_millis?` | [`DurationValue`](DurationValue.md)<UnitMillis> | This field should always be equal to zero in an _update_by_query response.
