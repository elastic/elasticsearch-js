# SecurityRemoteUserIndicesPrivileges
...
|----------|------|-------------|
| `field_security?` | `SecurityFieldSecurity[]` | The document fields that the owners of the role have read access to. |
...
| `query?` | `SecurityIndicesPrivilegesQuery[]` | Search queries that define the documents the user has access to. A document within the specified indices must match these queries for it to be accessible by the owners of the role. |
