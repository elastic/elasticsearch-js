...
| `action_modes?` | `Record<string, WatcherActionExecutionMode>` | Determines how to handle the watch actions as part of the watch execution. |
...
| `record_execution?` | `boolean` | When set to `true`, the watch record representing the watch execution result is persisted to the `.watcher-history` index for the current time.
...
| `watch?` | [`WatcherWatch`](WatcherWatch.md) | When present, this watch is used instead of the one specified in the request.
