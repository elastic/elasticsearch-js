...
| `boundary_max_scan?` | `integer` | How far to scan for boundary characters. |
| `boundary_scanner?` | [`SearchBoundaryScanner`](SearchBoundaryScanner.md) | Specifies how to break the highlighted fragments: chars, sentence, or word.
...
| `boundary_scanner_locale?` | `string` | Controls which locale is used to search for sentence and word boundaries.
This parameter takes a form of a language tag, for example: `"en-US"`, `"fr-FR"`, `"ja-JP"`. |
| `force_source?` | `boolean` | - |
| `fragmenter?` | [`SearchHighlighterFragmenter`](SearchHighlighterFragmenter.md) | Specifies how text should be broken up in highlight snippets: `simple` or `span`.
Only valid for the `plain` highlighter. |
| `fragment_size?` | `integer` | The size of the highlighted fragment in characters. |
| `highlight_filter?` | `boolean` | - |
| `highlight_query?` | [`QueryDslQueryContainer`](QueryDslQueryContainer.md) | Highlight matches for a query other than the search query.
...
| `max_analyzed_offset?` | `integer` | If set to a non-negative value, highlighting stops at this defined maximum limit.
...
| `no_match_size?` | `integer` | The amount of text you want to return from the beginning of the field if there are no matching fragments to highlight. |
| `number_of_fragments?` | `integer` | The maximum number of fragments to return.
...
| `order?` | [`SearchHighlighterOrder`](SearchHighlighterOrder.md) | Sorts highlighted fragments by score when set to `score`.
...
| `phrase_limit?` | `integer` | Controls the number of matching phrases in a document that are considered.
