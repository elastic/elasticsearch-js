...
|----------|------|-------------|
| `do_sample?` | [`float`](float.md) | For a `completion` task, instruct the inference process to perform sampling.
...
| `temperature?` | [`float`](float.md) | For a `completion` task, control the apparent creativity of generated completions with a sampling temperature.
It must be a number in the range of 0.0 to 2.0.
It should not be used if `top_p` is specified. |
| `top_p?` | [`float`](float.md) | For a `completion` task, make the model consider the results of the tokens with nucleus sampling probability.
