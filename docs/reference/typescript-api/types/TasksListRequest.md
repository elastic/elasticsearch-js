...
| `group_by?` | [`TasksGroupBy`](TasksGroupBy.md) | A key that is used to group tasks in the response.
...
| `parent_task_id?` | [`Id`](Id.md) | A parent task identifier that is used to limit returned information.
...
| `timeout?` | [`Duration`](Duration.md) | The period to wait for each node to respond.
