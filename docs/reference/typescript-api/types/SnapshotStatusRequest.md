...
|----------|------|-------------|
| `repository?` | [`Name`](Name.md) | The snapshot repository name used to limit the request.
It supports wildcards (`*`) if `<snapshot>` isn't specified. |
| `snapshot?` | [`Names`](Names.md) | A comma-separated list of snapshots to retrieve status for.
