...
| `privileges` | `SecurityIndexPrivilege[]` | A list of the privileges that you want to check for the specified indices. |
| `allow_restricted_indices?` | `boolean` | This needs to be set to `true` (default is `false`) if using wildcards or regexps for patterns that cover restricted indices.
