...
class TestRunner {
  ...
  async cleanupXPack () {
    ...
    try {
      const { body } = await this.client.xpack.security.getUser()
    ...
    try {
      const { body } = await this.client.xpack.security.getPrivileges()
      const privileges = []
      ...
      await helper.runInParallel(this.client, 'xpack.security.deletePrivileges', privileges)
    } catch (err) {
      this.tap.error(err, 'should not error: security privileges cleanup')
    }

    try {
      await this.client.xpack.ml.stopDatafeed({ datafeedId: '*', force: true })
      const { body } = await this.client.xpack.ml.getDatafeeds({ datafeedId: '*' })
    ...
    try {
      await this.client.xpack.ml.closeJob({ jobId: '*', force: true })
      const { body } = await this.client.xpack.ml.getJobs({ jobId: '*' })
    ...
    try {
      const { body } = await this.client.xpack.rollup.getJobs({ id: '_all' })
